...
export const PixelMao = () => {
  ...
  // Helper: Get symmetrical indices
  const getSymmetryIndices = (index: number) => {
    ...
    return [...new Set(indices)]; // Remove duplicates (center lines)
  };
  ...
  // Helper: Circle (Midpoint Circle Algorithm)
  const getCirclePixels = (x0: number, y0: number, x1: number, y1: number) => {
    ...
    addPoints(x0, y0, r, 0); // Cardinal points
    return pixels;
  };
  ...
  const handlePixelDown = (index: number) => {
    ...
    if (tool === 'eyedropper') {
      ...
      setTool('pencil'); // Auto-switch back to pencil
      setIsDrawing(false);
      return;
    }
    ...
    if (tool === 'fill') {
      ...
      pushState(newPixels);
      setIsDrawing(false); // Fill is an instant action
      return;
    }
    ...
    if (tool === 'line' || tool === 'rect' || tool === 'circle') {
      ...
      // We update state immediately for pencil/eraser to feel responsive
      // We DON'T push state yet, we push on MouseUp
      setHistory(prev => {
        const newHist = [...prev];
  ...
  const handlePixelMove = (index: number) => {
    ...
    if (tool === 'pencil' || tool === 'eraser') {
      ...
      // Apply symmetry to shape itself
  ...
  const handlePixelUp = () => {
    ...
    if (tool === 'line' || tool === 'rect' || tool === 'circle') {
      ...
      // Pencil/Eraser: State was mutated in place for perf, now we formally push it to history
